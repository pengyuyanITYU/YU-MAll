...
class ProductPublicAdminEndpointExposureTest {

    @Test
    void shouldNotExposePublicAdminControllers() throws ClassNotFoundException {
        List<String> adminMappings = findPublicAdminMappings("com.yu.item.controller", "com.yu.comment.controller");
    ...
    private static List<String> findPublicAdminMappings(String... basePackages) throws ClassNotFoundException {
        ...
        for (String basePackage : basePackages) {
            for (var candidate : scanner.findCandidateComponents(basePackage)) {
                Class<?> controllerClass = ClassUtils.forName(candidate.getBeanClassName(), ProductPublicAdminEndpointExposureTest.class.getClassLoader());
