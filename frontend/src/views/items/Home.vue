              ...
              <el-card class="product-card" shadow="hover" :body-style="{ padding: '0px' }">
                ...
                <div class="card-details" @click="enterItemDetail(item.id)">
                  ...
                  <div class="meta-row">
                    ...
                    <span class="sales">{{  item.sold }}人付款</span>
                  </div>
                  <div class="tag-row">
...
import {
  ...
  Compass, Odometer, Reading, Collection
...
const loadData = async () => {
  ...
  try {
    const sortParams = (() => {
      if (queryParams.sort === 'price_asc') return { sortBy: 'price', isAsc: true };
      if (queryParams.sort === 'price_desc') return { sortBy: 'price', isAsc: false };
      return {};
    })();

    // ★★★ 修复点：显式添加 category 和 brand 参数 ★★★
    const params = {
      pageNo: queryParams.pageNo,
      pageSize: queryParams.pageSize,
      name: queryParams.name || (route.query.q as string) || '',
      category: queryParams.category, // 确保传给后端
      brand: queryParams.brand,       // 确保传给后端
      minPrice: queryParams.minPrice,
      maxPrice: queryParams.maxPrice,
      sold: queryParams.sold,
      ...sortParams,
    };
...
// --- UI 数据 ---
// 新增首页10大分类（保持数据结构不变：{ name, icon }）
const categories = [
  { name: '服装与配饰', icon: ShoppingBag }, // 新增分类：服装与配饰
  { name: '电子产品', icon: Iphone }, // 新增分类：电子产品
  { name: '家居用品', icon: Coffee }, // 新增分类：家居用品
  { name: '美妆个护', icon: Star }, // 新增分类：美妆个护
  { name: '食品与饮料', icon: Sugar }, // 新增分类：食品与饮料
  { name: '户外运动', icon: Compass }, // 新增分类：户外运动
  { name: '母婴用品', icon: Goods }, // 新增分类：母婴用品
  { name: '汽车用品', icon: Odometer }, // 新增分类：汽车用品
  { name: '图书音像', icon: Reading }, // 新增分类：图书音像
  { name: '珠宝首饰', icon: Collection }, // 新增分类：珠宝首饰
];
const banners = [
