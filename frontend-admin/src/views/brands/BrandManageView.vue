<template>
    ...
    <div class="toolbar">
      <el-form :inline="true" :model="query">
        <el-form-item label="品牌名称">
          <el-input v-model="query.name" clearable placeholder="输入品牌关键词" />
        </el-form-item>
        <el-form-item>
  ...
  <el-dialog v-model="dialogVisible" :title="form.id ? '编辑品牌' : '新增品牌'" width="420px">
    <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
      <el-form-item label="品牌名称" prop="name">
        <el-input v-model="form.name" maxlength="20" show-word-limit placeholder="请输入品牌名称" />
      </el-form-item>
    </el-form>
    <template #footer>
...
async function load() {
  loading.value = true;
  try {
    const res = await listBrands(query);
    rows.value = res.rows || [];
...
async function submit() {
  ...
  try {
    const res = form.id
      ? await updateBrand({ id: form.id, name: form.name })
...
async function remove(row: BrandModel) {
  await ElMessageBox.confirm(`确认删除品牌「${row.name}」吗？`, '提示', {
    type: 'warning'
  });
